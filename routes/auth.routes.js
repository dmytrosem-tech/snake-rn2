...
router.post(
  ...
  [
    ...
    check("password", "Min passw 6 symbols").isLength({ min: 6 }),
  ],
  async (req, res) => {
    try {
      ...
      if (candidate) {
        return res.status(400).json({ message: "User is here now" });
      }
...
router.post(
  ...
  [
    ...
    check("password", "Go do you pass").exists(),
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        return res.status(400).json({
          errors: errors.array(),
          message: "Incorrect data when login",
        });
      }
      const { email, password } = req.body;
